#!/usr/bin/env python3
"""
Create Standalone Portfolio Dashboard
Embeds all JSON data directly into the HTML file for easy sharing
"""
...
def load_all_data():
    """Load all JSON data from API files"""
    output_dir = Path("output/api")
    
    data = {}
    files = ["summary.json", "trending.json", "established.json", "hidden_gems.json"]
...
<body>
    ...
    <div class="container-fluid px-4">
        ...
        <div class="search-container">
            ...
            <input type="text" class="search-input" id="search-input" placeholder="Search repositories by name, description, or topics...">
        </div>
        ...
        <div class="tab-content" id="leaderboardTabsContent">
            ...
            <div class="tab-pane fade show active" id="trending" role="tabpanel">
                <div id="trending-repos" class="row">
                    <div class="col-12 text-center py-5">
                        ...
                        <p class="mt-3">Loading trending repositories...</p>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="established" role="tabpanel">
                <div id="established-repos" class="row">
                    <div class="col-12 text-center py-5">
                        <div class="loading-spinner"></div>
                        <p class="mt-3">Loading established repositories...</p>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-pane fade" id="hidden-gems" role="tabpanel">
                <div id="hidden-gems-repos" class="row">
                    <div class="col-12 text-center py-5">
                        <div class="loading-spinner"></div>
                        <p class="mt-3">Loading hidden gems...</p>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        window.portfolioData = {json.dumps(data, indent=8)};
        ...
        async function loadData() {{
            try {{
                ...
                // Update statistics
                if (data.summary) {{
                    document.getElementById('total-repos').textContent = data.summary.total_repositories || 0;
                    document.getElementById('trending-count').textContent = data.summary.categories?.trending || 0;
                    document.getElementById('established-count').textContent = data.summary.categories?.established || 0;
                    document.getElementById('hidden-gems-count').textContent = data.summary.categories?.hidden_gems || 0;
    ...
    output_file = Path("output") / "ai_leaderboard_portfolio.html"
...
def main():
    """Main function"""
    print("🚀 Creating Standalone Portfolio Dashboard...")
    
    html_file = create_standalone_html()
    
    print(f"✨ Standalone Portfolio Dashboard created!")
    print(f"📁 File: {html_file}")
    print(f"🌐 Open: file://{html_file.absolute()}")
    print()
    print("🎯 This file is completely self-contained:")
    print("  • All data embedded directly in HTML")
    print("  • No external JSON files needed")
    print("  • Works offline")
    print("  • Perfect for sharing or hosting")
